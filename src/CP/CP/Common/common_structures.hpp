...
template<typename T>
dsSTRING_BUILDER& dsSTRING_BUILDER::Append(const T& arg, const char* delimiter)
{
	buff << arg << delimiter;
	return *this;
}
...
dsSTRING_BUILDER& dsSTRING_BUILDER::AppendFmt(const char* fmt, Args... args)
{
	static constexpr size_t maxAppendSize = 200;
	char str[maxAppendSize] = "";
	int ret = sprintf_s<maxAppendSize>(str, fmt, args...);
	if (ret < 0)
	{
...
template<typename T>
cudaARRAY2D<T>::cudaARRAY2D(std::size_t w, std::size_t h)
	: width(w)
	, height(h)
{
...
template<typename T>
void cudaARRAY2D<T>::MoveToDevice(T* from, std::size_t w, std::size_t h)
{
...
template<typename T>
void cudaARRAY2D<T>::MoveToHost(T* to, std::size_t w, std::size_t h) {
	ASSERT_MSG(w <= width && h <= height, "Sizes out of range: Width is %zd (original: %zd), Height is %zd (original: %zd)", w, width, h, height);
	ERROR_WRAPPER_CALL(cudaMemcpy2DToArray, to, 0, 0, data, w * sizeof(T), w * sizeof(T), h, cudaMemcpyDeviceToHost);
}

template <typename ... Args>
typename dsEVENT<Args...>::CallbackID dsEVENT<Args...>::Subscribe(CallbackType callback)
{
...
template <typename ... Args>
void dsEVENT<Args...>::Trigger(Args... args)
{
...
template <typename ... Args>
void dsEVENT<Args...>::Unsubscribe(CallbackID id)
{
	callbacks.erase(id);
}